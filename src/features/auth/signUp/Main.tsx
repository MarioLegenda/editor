...
export function Main() {
	const { current, set } = useSimpleUiState<
  ...
  >({
	...
	return (
		<div css={styles.root}>
			{current === 'isSignUpSuccess' && (
				<div css={styles.notification}>
					<Success
						disallowClose
						title="Sign up successful"
						message="You are now signed up. Confirmation email has been sent to your email."
					/>
				</div>
			)}

			{current === 'isSignUpError' && (
				<div css={styles.notification}>
					<Error />
				</div>
			)}
			...
			<Form
				onSuccess={() => set('isSignUpSuccess')}
				onError={() => set('isSignUpError')}
			/>
		</div>
	);
}
